...
def create_return(price_array, window):
    ...
    for i in range(window, len(price_array)):
        ...
        if i+window+1 == len(price_array): break
    return output
...
def main():
    ...
    for file in os.listdir(directory):
        ...
        if filename.endswith(".csv"):
            df_gc6 = pd.read_csv('data/'+str(filename))
            ...
            five_day_return = create_return(price_val, window = 5)
            ...
            raw_data = {'Settle_Price': price_val,
            'Daily_Return': daily_return, '5-day Return': five_day_return,'Volume': np.array(df['Volume'][::-1]),'SMA5' : sma_map["SMA5"],
